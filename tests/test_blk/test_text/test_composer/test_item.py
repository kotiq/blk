...
from blk.types import (BlockComment, Color, Float, Float12, Float2, Float3, Float4, Int, Int2, Int3, LineComment, Long,
                       Name, Str, Vector, true)
from blk.text.composer import item

def like(p, q):
    pn, pv = p
    qn, qv = q
    if isinstance(qv, Float) or isinstance(qv, Vector) and qv.type == Float:
        return pv.close_to(qv)
    else:
        return pv == qv


@mark.parametrize(['text', 'item_'], [
...
def test_compose_named_parameter(text, item_):
    n, v = parsed = item.parse(text)
    name, value = item_
    assert isinstance(n, Name)
    assert isinstance(v, value.__class__)
    assert like(parsed, item_)
...
def test_compose_comment(text, item_):
